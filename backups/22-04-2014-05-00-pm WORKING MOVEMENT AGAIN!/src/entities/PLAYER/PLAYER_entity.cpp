    ...
    if (key == 119 || key ==  1073741906 ) //if key == W then try to move forward.
    {
            ...
            for (int i=0; (i)<entities; ++i) //loop for the amount of instatiated entities.
            {
               ...
               int thistmpX = (this->return_locationX() * 10);
                ...
                {
                   std::cout << "entered the second loop thing\n";
                     int e = entities;
                    --e;
                ...
                else if (i == (entities - 1)) //if we reached the end of testying all the entities and still havent moved (if we move we break) then we can probably assume we can just move.
                {                             // This is however just covering up the fact that sometimes for some reason both previous "if" statements turn up as false.
                                              // What i have here is super hacky and not particually pretty at all but it works.
    ...
    if (key == 97 || key == 1073741904  ) //if key == A then try to move LEFT.
    {
            ...
            for (int i=0; (i)<entities; ++i) //loop for the amount of instatiated entities.
            {
               ...
               else if (tmpY != (thistmpY) && tmpX != (thistmpX - 1)) // If the entity we're looking at is not at the position we want to move to.
                {
                    ...
                    if (i == e) //and we're at the end of the entities
                    {
                        std::cout << "We're at the end of the entities, so move anyway.\n\n";
                        locationXmtx.lock();
                        this->locationX = locationX - 0.1f; //Then move.
                        locationXmtx.unlock();
                        break; 
                    }
		    std::cout << "left second if\n\n";
                }
                else if ( i  == (entities - 2 ))
                {
                    std::cout << "We reached the end of the entities here.\n\n";
                   std::cout << "We're at the end of the entities, so move anyway.\n\n";
                   locationXmtx.lock();
                   this->locationX = locationX - 0.1f; //Then move.
                   locationXmtx.unlock();
            ...
            for (int i=0; (i)<entities; ++i) //loop for the amount of instatiated entities.
            {
               ...
               // std::cout << "tmpY = " <<  tmpY << " tmpX = " <<  tmpX << " thistmpY = " << thistmpY << " thistmpX =" << thistmpX << "\n" ; 
                ...
                else if (tmpY != (thistmpY) && tmpX != (thistmpX + 1)) // If the entity we're looking at is not at the position we want to move to.
                {
                    ...
                    --e;
		    --e;
		    std::cout << "entered second loop (right)"; 
                ...
                {   
			std::cout << "end of entities, rendering...\n\n";
